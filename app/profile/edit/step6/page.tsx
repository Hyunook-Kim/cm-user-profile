...
function LifestyleTag({ label, isSelected, isDisabled, onClick, variant = "hobby" }: LifestyleTagProps) {
  // 취미 섹션: 미선택 시 회색 배경
  // 운동/관심사 섹션: 미선택 시 흰색 배경 + 회색 테두리
  const baseStyles = "flex items-center px-3 py-2 rounded-full text-caption-md transition-colors";

  let stateStyles = "";
  if (isSelected) {
    stateStyles = "bg-white border border-pink text-pink";
  } else if (isDisabled) {
    stateStyles = variant === "hobby"
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center justify-center rounded bg-navy px-2 py-2">
        <span className="text-caption-md text-white">선택 {currentCount}개</span>
      </div>
    </div>
  );
}
...
function CategoryCard({
  ...
  return (
    <div className={`flex flex-col gap-3 bg-white p-5 ${roundedClass} ${borderClass} border-gray-100`}>
    ...
    <>
      ...
      <main className="flex flex-col items-start gap-[18px] overflow-y-auto bg-gray-100 p-4 pb-[96px]">
        <FormProvider {...methods}>
          <Title title="라이프스타일" step={6} subtitle="*모든 항목이 필수입니다." />
          ...
          >
            ...
            <section className="flex flex-col gap-3">
              <SectionTitle title="취미" maxCount={5} currentCount={hobbies.length} />
            ...
            {/* Section 2: 운동 */}
            <section className="flex flex-col gap-3">
              <SectionTitle title="운동" maxCount={3} currentCount={exercises.length} />
              <div className="flex flex-col">
                {exerciseCateg.map((category, index) => (
                  <CategoryCard
                    ...
                    maxSelect={3}
                    onToggle={createToggleHandler("exercises", exercises, 3)}
                    variant="exercise"
                    isFirst={index === 0}
            ...
            {/* Section 3: 관심사 */}
            <section className="flex flex-col gap-3">
              <SectionTitle title="관심사" maxCount={3} currentCount={interests.length} />
              <div className="flex flex-col">
                {interestCateg.map((category, index) => (
                  <CategoryCard
                    ...
                    maxSelect={3}
                    onToggle={createToggleHandler("interests", interests, 3)}
                    variant="interest"
                    isFirst={index === 0}
